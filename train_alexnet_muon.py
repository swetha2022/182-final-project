...
class Config:
    ...
    exp_name = "vision_transformer_muon_fixed"
    batch_size = 128 
...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            checkpoint_path = os.path.join(config.save_dir, f"vision_transformer_muon_epoch_{epoch+1}.pth")
            torch.save({
                'epoch': epoch + 1,
