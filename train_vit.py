...
class Config:
    ...
    exp_name = "vision_transformer_scratch_baseline"
    batch_size = 128  
    learning_rate = 0.01
    momentum = 0.9
    weight_decay = 0.0005
...
def main():
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.ColorJitter(brightness=0.1, contrast=0.1, saturation=0.1, hue=0.05),  # RGB shifting
            transforms.ToTensor(),
            normalize,
        ])
    )
    ...
    for epoch in range(config.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            checkpoint_path = os.path.join(config.save_dir, f"vision_transformer_epoch_{epoch+1}.pth")
            torch.save({
                'epoch': epoch + 1,
